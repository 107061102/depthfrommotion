    ...
    def depth_caculation(self, xd, yd, zd, rx, ry, rz, real_rx, real_rz, frame):
        ...
        for i in range(len(contours_1)):
            if(cv2.contourArea(contours_1[i]) > 2000):
                showFrame = cv2.drawContours(showFrame, contours_1, i, (0,255,0), -1)
    
        for i in range(len(contours_2)):
            if(cv2.contourArea(contours_2[i]) > 2000):
                showFrame = cv2.drawContours(showFrame, contours_2, i, (255,0,0), -1)
    
        arr=np.zeros(5)
    
    

        if np.sum(showFrame[120:400:10,0:60:10,0] == 255)>25:
            cv2.rectangle(showFrame, (0,5), (60,475), (255,0,0), 3)
            arr[0] = 1
        elif np.sum(showFrame[120:400:10,0:60:10,1] == 255)>25:
            cv2.rectangle(showFrame, (0,5), (60,475), (0,255,0), 3)
            arr[0] = 2
        elif np.sum(showFrame[120:400:10,0:60:10,2] == 255)>25:
            cv2.rectangle(showFrame, (0,5), (60,475), (0,0,255), 3)
            arr[0] = 3
        
        if  np.sum(showFrame[120:400:10,60:180:10,0] == 255)>40:
            cv2.rectangle(showFrame, (60,5), (180,475), (255,0,0), 3)
            arr[1] = 1
        elif np.sum(showFrame[120:400:10,60:180:10,1] == 255)>40:
            cv2.rectangle(showFrame, (60,5), (180,475), (0,255,0), 3)
            arr[1] = 2
        elif np.sum(showFrame[120:400:10,60:180:10,2] == 255)>40:
            cv2.rectangle(showFrame, (60,5), (180,475), (0,0,255), 3)
            arr[1] = 3
        
        if  np.sum(showFrame[120:400:10,180:460:10,0] == 255)>50:
            cv2.rectangle(showFrame, (180,5), (460,475), (255,0,0), 3)
            arr[2] = 1
        elif np.sum(showFrame[120:400:10,180:460:10,1] == 255)>50:
            cv2.rectangle(showFrame, (180,5), (460,475), (0,255,0), 3)
            arr[2] = 2
        elif np.sum(showFrame[120:400:10,180:460:10,2] == 255)>50:
            cv2.rectangle(showFrame, (180,5), (460,475), (0,0,255), 3)
            arr[2] = 3
        
        if  np.sum(showFrame[120:400:10,460:580:10,0] == 255)>40:
            cv2.rectangle(showFrame, (460,5), (580,475), (255,0,0), 3)
            arr[3] = 1
        elif np.sum(showFrame[120:400:10,460:580:10,1] == 255)>40:
            cv2.rectangle(showFrame, (460,5), (580,475), (0,255,0), 3)
            arr[3] = 2
        elif np.sum(showFrame[120:400:10,460:580:10,2] == 255)>40:
            cv2.rectangle(showFrame, (460,5), (580,475), (0,0,255), 3)
            arr[3] = 3
            
        if  np.sum(showFrame[120:400:10,580:640:10,0] == 255)>25:
            cv2.rectangle(showFrame, (580,5), (640,475), (255,0,0), 3)
            arr[4] = 1
        elif np.sum(showFrame[120:400:10,580:640:10,1] == 255)>25:
            cv2.rectangle(showFrame, (580,5), (640,475), (0,255,0), 3)
            arr[4] = 2
        elif np.sum(showFrame[120:400:10,580:640:10,2] == 255)>25:
